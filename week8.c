...
void handle_entry_process(char *entry_name){
    char *destiation_file = string_safe_alloc(PATH_MAX);
    sprintf(destiation_file, "%s/%s_statistica.txt", destiation_directory, entry_name);
    ...
    check_entry_type(entry_name);
    system_function_error_wrapper("Function call error: 'close'", close(fw));
    exit(line_counter);
}
...
void child_process_factory(DIR *dir){
    errno = 0;
    ...
    if(!entry) return;
    if(is_relative_path(entry->d_name)) return child_process_factory(dir);
    pid_t pid = fork();
    system_function_error_wrapper("Failed to create child process", pid);
    child_process_counter++;
    if(!pid){
        handle_entry_process(entry->d_name);
    }else{
        if(is_bit_map_file(entry->d_name)){
            pid = fork();
            system_function_error_wrapper("Failed to create child process", pid);
            child_process_counter++;
            if(!pid) discolor_bmp_file_process(entry->d_name);
        }
        return child_process_factory(dir);
    }
}
